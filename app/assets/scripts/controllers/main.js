    ...
    function ($scope, Messages, Popups, ProjectsService, AuthService, MapsService, $timeout, $sails, consts, blockFactory, $localStorage) {
      ...
      var execute_map = function (map, isResume) {
        ...
        var success = function (result) {
            ...
            $scope.map.versions[$scope.map.versionIndex].executions.push(result.resObj);
          ...
          error = function (err) {
            ...
            $scope.button_text = 'execute';
            $scope.btn_disabled = false;
            $scope.map.versions[$scope.map.versionIndex].executions.push(err);
      ...
      $scope.jsTreeContextMenu = function (node) {
        var items = {
          renameMap: {
            ...
            "action": function () {
              Popups.open({
                ...
                $('js-tree').jstree(true).rename_node(node, map.name);
              });
            }
          },
          ShowMapVersions: {
          ...
          ShowMapExecutionVersions: {
            type: "map",
            "label": "Show Executions",
            "action": function (obj) {
              Popups.open({
                templateUrl: 'views/Popups/ShowMapExecutions.html',
                controller: 'MapExecutionsCtrl',
                resolve: {map: node.original}
              }, function (versionIndex) {
                console.log("exit from show execution version");
              });
            }
          },
          DeleteMap: {
      ...
      $scope.loadMapVersion = function (index) {
        ...
        for (var key in $scope.map.mapView.attributes) {
          $scope.attributes.push($scope.map.mapView.attributes[key]);
        }
      ...
      $scope.saveAttribute = function (attr) {
        ...
        $scope.map.mapView.attributes[attr.name] = attr;
      }
