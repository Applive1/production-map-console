...
angular.module('productionMapConsoleApp').controller('BaseExecutionCtrl', ['$scope', '$window', '$uibModalInstance', 'BaseAgentService', 'execution',function ($scope, $window, $uibModalInstance, BaseAgentService, execution) {
        ...
        function getAgentsStatuses(){
            ...
            for(var key in $scope.agentsNames){
                ...
                if(attrValue){
                    ...
                    var res = $scope.execution.resObj[attrName].result;
                    if(!(res in status)){
                        status[res] = [];
                    }
                    status[res].push(attrName);
                }
                else{
                    status.didntRun.push(attrName);
                }
