...
export class DraggableDirective implements OnDestroy, OnInit {
  ...
  doTranslation(x: number, y: number) {
    ...
    this.renderer.setElementStyle(this.el.nativeElement, 'left', ((x - this.Δx) + (this.el.nativeElement.offsetWidth / parseFloat("2.0"))) + 'px');
    console.log(((x - this.Δx) + (this.el.nativeElement.offsetWidth / 2.0)));
  }
